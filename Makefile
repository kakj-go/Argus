.PHONY: fmt test vet run-server contract-lint contract-generate contract-check contract-breaking check-production-artifacts migrate sqlc e2e-m2-k8s e2e-m3-k8s e2e-m4-k8s
...
contract-generate:
	...
	@set -e; for domain in common identity authorization labels action card agent stream telemetry setup platform enterpriseidentity enterpriseauthz machine audit secretapi hostapi kubernetesapi connectionapi actionapi connectorapi conversationapi modelapi workflowapi automationapi sandboxapi; do \
	...
	@set -e; for domain in setup platform enterpriseidentity enterpriseauthz machine audit secretapi hostapi kubernetesapi connectionapi actionapi connectorapi conversationapi modelapi workflowapi automationapi sandboxapi; do \
	...
	@set -e; for domain in enterpriseauthz secretapi hostapi kubernetesapi connectionapi actionapi connectorapi sandboxapi; do \
		node scripts/split-generated-go-server.mjs internal/gen/openapi/$$domain/server.gen.go; \
	done
	node scripts/generate-contract-index.mjs web/packages/api-client/src/generated/contracts.ts common identity authorization labels action card agent stream telemetry setup platform enterpriseidentity enterpriseauthz machine audit secretapi hostapi kubernetesapi connectionapi actionapi connectorapi conversationapi modelapi workflowapi automationapi sandboxapi
