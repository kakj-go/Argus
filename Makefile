.PHONY: fmt test vet run-server contract-lint contract-generate contract-check contract-breaking check-production-artifacts migrate sqlc otelcol-linux-arm64 otelcol-windows-amd64 otelcol-distributions e2e-m2-k8s e2e-m3-k8s e2e-m4-k8s e2e-m5-k8s e2e-m6-k8s e2e-m7-k8s
...
contract-generate:
	...
	@set -e; for domain in setup platform enterpriseidentity enterpriseauthz machine audit secretapi hostapi kubernetesapi connectionapi actionapi connectorapi conversationapi modelapi workflowapi automationapi sandboxapi cardapi remoteaccessapi telemetryapi; do \
