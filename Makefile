.PHONY: fmt test vet run-server contract-lint contract-generate contract-check contract-breaking migrate sqlc e2e-m2-k8s
...
contract-generate:
	...
	@set -e; for domain in common identity authorization labels action card agent stream telemetry setup platform enterpriseidentity enterpriseauthz machine audit; do \
	...
	@set -e; for domain in setup platform enterpriseidentity enterpriseauthz machine audit; do \
	...
	node scripts/split-generated-go-server.mjs internal/gen/openapi/enterpriseauthz/server.gen.go
	node scripts/generate-contract-index.mjs web/packages/api-client/src/generated/contracts.ts common identity authorization labels action card agent stream telemetry setup platform enterpriseidentity enterpriseauthz machine audit
