...
func selectTool(request replayRequest) (string, string, bool) {
	...
	rawPrompt := lastContent(append(request.Messages, request.Input...))
	prompt := strings.ToLower(rawPrompt)
	trimmedPrompt := strings.TrimSpace(prompt)
	if strings.HasPrefix(trimmedPrompt, "tool result:") || strings.HasPrefix(trimmedPrompt, "verify this deterministic execution result") {
		return "", "", false
	}
