...
| 企业用户/部门/角色/ServiceAccount/explicit resource authorization | OpenAPI 普遍有 name 128、description 1024、数组唯一性和 UUID                       | explicit resource authorization 及 Label Selector 的对象/标量约束均由生成文件消费      | 普通字段边界已收敛；跨字段和引用关系仍由领域层负责                |
