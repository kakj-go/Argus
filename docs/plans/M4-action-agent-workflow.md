...
- `argus-worker` 使用 `agent`、`action`、`compaction`、`sandbox` 四个独立 PostgreSQL Task Queue 和 Processor；Evaluation 由一个 `--pool=default` Deployment 同时运行四类任务，Local Hardening/Production 使用四个拆分 Deployment。PostgreSQL Task Lease/Fence 是唯一领取事实，Redis 只负责唤醒。
...
- 当前版本不提供定时无人值守任务；写操作统一使用 PendingAction、Approval、Execution 和 Task 状态机。审批收件箱的正式产品体验仅覆盖桌面端。
...
- [x] `M4-APPROVAL-INBOX-01` 完成桌面审批收件箱一级/二级 Tab、URL 深链接、服务端 `scope`、签名游标分页和远程访问审批隔离。
...
- `result_unknown` 对账使用独立 ConnectorCommand 终态事实；未知期间不重放 Host 变更，命令成功后 PendingAction、Execution 和 Task 一次性收敛。
...
- 桌面审批收件箱必须覆盖“待我审批 / 我发起的 / 已处理”、刷新恢复、审批结果、执行状态、远程访问隔离和待办徽标范围过滤。
...
截至 2026-08-17，上述门禁已由旧 Shell Harness 全部通过。最终成功运行号为 `20260817144832-31660`，脱敏证据位于 `artifacts/m4-e2e/20260817144832-31660`；测试覆盖双策略审批、可信 Run 绑定、ResultUnknown 不重放、模型额度耗尽、Sandbox 配额、Redis 停止与 Server 恢复和 real Playwright，结束后临时 Namespace、PVC 和 Lease 已清理。
...
- AI、Card 和 OpenSandbox 不能消费人工远程会话票据。
