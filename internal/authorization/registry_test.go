...
func TestPermissionRegistryVersionAndBuiltinRolesIncludeM4(t *testing.T) {
	if PermissionRegistryVersion != 3 {
		t.Fatalf("permission registry version = %d, want 3", PermissionRegistryVersion)
	}

	for _, permission := range []string{
		"conversation.use", "model.manage", "model.quota.manage", "approval_policy.manage",
		"approval.decide", "execution.read", "automation.manage",
	} {
	...
	assertRolePermissions(t, "resource_approver", "approval.read", "approval.decide", "execution.read")
}
