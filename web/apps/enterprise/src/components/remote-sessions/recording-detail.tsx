    ...
    >
      {recordingId !== null &&
        (meta.isPending ? (
          ...
          <Alert
            description={t("remoteSessions.unavailable")}
            title={t("remoteSessions.recordingTitle")}
            tone="danger"
          />
        ) : (
          recording && (
            <div className="argus-recording-detail">
              {recording.status === "recording" && (
                <Alert
                  description={t(
                    "remoteSessions.recordingInProgressDescription",
                  )}
                  title={t("remoteSessions.recordingInProgress")}
                  tone="warning"
                />
              )}
              <dl className="argus-recording-meta">
                <div>
                  <dt>{t("remoteSessions.columns.session")}</dt>
                  <dd>{recording.session_id}</dd>
                </div>
                <div>
                  <dt>{t("remoteSessions.columns.status")}</dt>
                  <dd>
                    <StatusBadge
                      tone={
                    ...
                    >
                      {t(`remoteSessions.status.${recording.status}`)}
                    </StatusBadge>
                  </dd>
                </div>
                <div>
                  <dt>{t("remoteSessions.columns.chunks")}</dt>
                  <dd>{recording.chunk_count}</dd>
                </div>
                <div>
                  <dt>{t("remoteSessions.columns.size")}</dt>
                  <dd>{recording.size_bytes.toLocaleString(locale)} B</dd>
                </div>
                <div>
                  <dt>{t("remoteSessions.columns.retention")}</dt>
                  <dd>
                    {new Date(recording.retention_until).toLocaleString(locale)}
                  </dd>
                </div>
                <div>
                  <dt>{t("remoteSessions.recordingDuration")}</dt>
                  <dd>
                    {t("remoteSessions.recordingDurationValue", {
                      seconds: Math.round(duration),
                    })}
                  </dd>
                </div>
                <div className="argus-recording-meta__wide">
                  <dt>{t("remoteSessions.hashChain")}</dt>
                  <dd>
                    <code>{recording.final_hash ?? "-"}</code>
                  </dd>
                </div>
              </dl>
              {isError && (
