...
export function SettingsInteractiveCardsPage() {
  ...
  const enterprise = (cards.data ?? []).filter(
    (card) => card.source === "enterprise",
  );
  ...
  return (
    ...
    >
      ...
      {detail && (
        <CardDetailDrawer
          card={detail}
          editable={admin && detail.source === "enterprise"}
      ...
      {cards.map((card) => (
        <article className="argus-ic-row" key={card.id}>
          <div className="argus-ic-row__meta">
            <div>
              <b>{card.name}</b>
              <Badge>{card.version}</Badge>
            </div>
            <p>{card.description}</p>
            <span>
              <StatusBadge tone={card.enabled ? "success" : "neutral"}>
                {t(`aiSettings.cards.${card.lifecycle}`)}
              </StatusBadge>{" "}
              · {card.slug} · rev {card.revision}
            </span>
          </div>
          <LazyCardPreview card={card} />
          <div className="argus-ic-row__actions">
            {!editable && card.source === "system" && (
              <LockKeyhole
...
function LazyCardPreview({ card }: { card: InteractiveCard }) {
  ...
  const contract = provisionalCardContract({
    card_id: card.id,
    revision: card.revision,
    card_instance_id: `preview-${card.id}`,
    locale,
  ...
  return (
    <div
      className={`argus-ic-preview ${expanded ? "is-expanded" : ""}`}
      ref={hostRef}
    >
      {visible && (
        <SandboxCard
          ...
          color_scheme={resolvedTheme}
          html={card.htmlTemplate}
          initial_data={card.demoData}
          locale={locale}
          manifest={contract.manifest}
          max_height={expanded ? 1200 : 320}
          min_height={120}
...
function CardDetailDrawer({
  ...
  const validate = useMutation({
    mutationFn: () => api.interactiveCards.validate(card.id),
    onSuccess: () => void refresh(),
  });
  const toggle = useMutation({
    mutationFn: () =>
      card.enabled
        ? api.interactiveCards.disable(card.id)
        : api.interactiveCards.enable(card.id),
    onSuccess: (next) => {
      setCard(next);
      void queryClient.invalidateQueries({ queryKey: ["interactiveCards"] });
    },
  });
  return (
    <>
        ...
        footer={
          editable ? (
            <>
              ...
              <Button
                disabled={!card.enabled && !card.validation?.valid}
          ...
          <div className="argus-ic-validation">
            <StatusBadge tone={card.validation?.valid ? "success" : "warning"}>
              {card.validation?.valid
                ? t("aiSettings.cards.validationPassed")
                : t("aiSettings.cards.validationFailed")}
            </StatusBadge>
            {(card.validation?.issues ?? []).map((issue) => (
              <small key={`${issue.code}-${issue.slot ?? ""}`}>
                {issue.code}: {issue.slot ?? issue.message}
              </small>
            ))}
          </div>
          <section>
            ...
            <div className="argus-ic-slots">
              {card.slots.map((slot) => (
                <button
                  className={slotName === slot.name ? "is-active" : ""}
                  disabled={!editable || slot.aiGenerated}
                  key={slot.name}
                  onClick={() => setSlotName(slot.name)}
                  type="button"
                >
                  <Link2 size={13} />
                  <span>{slot.name}</span>
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      const binding: SlotBinding = {
        ...
        fieldPath: fieldPath || schema.fields[0]?.path || "",
      };
      return api.interactiveCards.updateBindings(card.id, [
        ...card.bindings.filter((entry) => entry.slotName !== slotName),
        binding,
      ]);
    },
    onSuccess: onSaved,
    ...
    >
      <div className="argus-settings-form">
        <Field label={t("aiSettings.cards.bindingMode")}>
          <Select
            onValueChange={(value) => setMode(value as SlotBindingMode)}
            options={[
        ...
        <Field label={t("aiSettings.cards.tool")}>
          <Select
            ...
            options={(schemas.data ?? []).map((schema) => ({
              value: schema.toolName,
              label: `${schema.toolName} · ${schema.version}`,
            }))}
            value={currentSchema?.toolName ?? ""}
          />
        </Field>
        <Field label={t("aiSettings.cards.field")}>
          <Select
            onValueChange={setFieldPath}
            options={(currentSchema?.fields ?? []).map((field) => ({
              value: field.path,
              label: `${field.path} · ${field.type}`,
            }))}
            value={fieldPath || currentSchema?.fields[0]?.path || ""}
          />
        </Field>
      </div>
    </FormDrawer>
