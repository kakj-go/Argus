...
export function createOrgSeed(now: number): OrgSeed {
  ...
  return {
    ...
    ] satisfies Array<Omit<RoleBinding, "version" | "updated_at">>).map(
      (binding) => ({ ...binding, version: 1, updated_at: binding.created_at }),
    ),
    dataScopes: [
      {
        id: "ds-ops-nonprod",
    ...
    approvalPolicies: [
      {
        ...
        enterpriseId: "ent-acme",
        name: "生产危险操作审批",
        description: "production 环境 dangerous/critical 操作需 resource_operator 审批",
